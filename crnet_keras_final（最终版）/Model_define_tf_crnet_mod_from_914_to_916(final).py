...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 16 21:21:25 2020
...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 14 18:12:25 2020
...
def convbn_no_bn(input_x,output,conv): #convbn(x,2,3)
    x = layers.Conv2D(output, conv, padding = 'SAME')(input_x)
    #x = layers.BatchNormalization()(x)
    x = swish(x)
    return x
...
def encoder2(input_x): #convbn(x,2,3)
    x1 = convbn(input_x,64,5)
    x1 = convbn(x1,128,5)
    x1 = convbn(x1,256,5)
    
    return x1

def encoder3(input_x): #convbn(x,2,3)
    x1 = convbn(input_x,64,3)
    x1 = convbn(x1,128,3)
    x1 = convbn(x1,256,3)
...
def residual_block(y):
    ...
    y1 = Conv2D(256, (5, 5), padding='same')(y)
...
def RefineNet_no_bn(input_x):  # convbn(x,2,3)
    x1 = convbn_no_bn(input_x, 32, 3)
    x1 = convbn_no_bn(x1, 16, 5)
    x1 = convbn_no_bn(x1, 32, 5)
    x1 = convbn_no_bn(x1, 64, 3)
