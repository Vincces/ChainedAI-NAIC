...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 10 17:40:39 2020
...
def ecn(input_x,output,conv): #convbn(x,2,3)
    ...
    x = layers.Conv2D(output, conv, padding = 'SAME',activation="relu")(x)
    x = layers.BatchNormalization()(x)
    x = layers.LeakyReLU(0.3)(x)
    return x
...
def Encoder(x,feedback_bits):
    ...
    with tf.compat.v1.variable_scope('Encoder'):
        ...
        encoder_output = layers.Dense(units=128, activation='sigmoid')(x)
        encoder_output = K.round(encoder_output)
...
def Decoder(x,feedback_bits):
    ...
    #x = layers.Dense(256*2*4, activation='sigmoid')(x) 
    x = layers.Dense(64*4*8)(x) 
    #print(x)
    x = layers.Reshape((4, 8, 64))(x)
    #x = convbn(x, 512, 2)
    #x = dn(x, 256, 3)
    x = dn(x, 128, 7)
    x = dn(x, 64, 9)
